...
In the high-$AI$ region of the efficiency landscape, there exist at least three distinct attractor types:
\begin{enumerate}[label=\textbf{A\arabic*.}]
\item \textbf{Integrated Information Attractor} ($\Phi$-attractor): Systems maximizing integrated information $\Phi$ (IIT-type consciousness)
\item \textbf{Predictive Processing Attractor} ($\mu$-attractor): Systems maximizing prediction accuracy through hierarchical models (FEP-type cognition)
\item \textbf{Distributed Coordination Attractor} ($\zeta$-attractor): Systems maximizing distributed coordination without centralized integration (collective intelligence)
\end{enumerate}
...
\begin{lemma}[Physical Self-Measurement Bound]
...
This is a physical limit, not a logical one (addressing CV-4). \hfill $\square$
\end{proof}
...
\begin{enumerate}[label=(\roman*)]
\item \textbf{CV-1 Resolution}: KID is treated as a dimensionless ratio throughout, avoiding dimensional inconsistency.
\item \textbf{CV-2 Resolution}: Consciousness is demoted from ``necessary consequence'' to ``one attractor among many.'' High $AI$ implies complex information processing, not any specific phenomenology.
\item \textbf{CV-3 Resolution}: Kolmogorov complexity $K(\cdot)$ replaces semantic information, grounding the framework in algorithmic information theory.
\item \textbf{CV-4 Resolution}: Physical self-measurement limits replace G\"odelian incompleteness, providing a thermodynamic bound on self-knowledge.
\end{enumerate}
